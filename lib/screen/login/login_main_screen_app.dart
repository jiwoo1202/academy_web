...
class LoginMainScreenApp extends StatefulWidget {
  static final String id = '/login_main_app';

  const LoginMainScreenApp({Key? key}) : super(key: key);

  @override
  State<LoginMainScreenApp> createState() => _LoginMainScreenAppState();
}

class _LoginMainScreenAppState extends State<LoginMainScreenApp>
  ...
  // @override
  // void initState() {
  //   _nestedTabController = TabController(length: 2, vsync: this,initialIndex: 0);
  //   initialization();
  ...
  //   print('save id check : ${_saveIdStr}');
  //
  //   if (userInfo != null) {
  //     // 로그인
...
void initState() {
  ...
  Future.delayed(Duration.zero,(){
    showConfirmTapDialog(context, 'dsdas', () {
      Get.offNamedUntil(
        LandingPageMain.id,
            (route) => false,
      );
                ...
                GestureDetector(
                  onTap: () {
                    showComponentDialog(context, '로그아웃을 하시겠습니까?', () {
                      Get.offAllNamed(LoginMainScreenApp.id);
